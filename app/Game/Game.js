...
class Game {
    ...
    constructor() {
        ...
        this.score = 0;
        let score_html = document.querySelector("scoreli");
    ...
    checkCollisions() {
        ...
        for (let i = 0; i < this.opponentShots.length; i++) {
            impact =
                impact || this.hasCollision(this.player, this.opponentShots[i]);
        }
        ...
        for (let i = 0; i < this.playerShots.length; i++) {
            killed =
                killed || this.hasCollision(this.opponent, this.playerShots[i]);
        }
        if (killed) {
            this.opponent.die();
        }
